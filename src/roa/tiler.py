"""
Based on https://github.com/SEE-GEO/ccic/blob/6899713fa507efffe7b759b8712c2650fe657374/ccic/tiler.py

Note:
The weights are not perfect, they exhibit a 'reticle effect',
but they are good enough for our goals.
"""
...
class Tiler:
    ...
    def __init__(self, x, tile_size=128, overlap=64, wrap_columns=False):
        """
        Args:
    ...
    def assemble(self, slices):
        ...
        if isinstance(slice_0, torch.Tensor):
            results = torch.zeros(shape).type_as(slice_0)
        else:
        ...
        for i, row in enumerate(slices):
            ...
            for j, slc in enumerate(row):
                ...
                results[..., row_slice, col_slice] += slc * (
                    self.get_weights(i, j).astype(slc.dtype) if isinstance(slc, np.ndarray) else torch.from_numpy(self.get_weights(i, j)).type_as(slc)
                )
